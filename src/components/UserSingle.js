...
function UserSingle() {
  ...
  useEffect(() => {
    if (userCtx.usersList) {
      const userInfo = userCtx.usersList.filter(
  ...
  useEffect(() => {
    if (userCtx.usersList) {
      setPurchasedToken(
        userCtx.usersList.filter((user) => user.account === address)[0].balance,
      )
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userCtx.usersList])
                  ...
                  >
                    ...
                    {userCtx.whiteList &&
                      userCtx.whiteList.some(
                        (user) => user.address === usersList.account,
                      ) && (
                        <MdVerified
                ...
                <div className="card-body">
                  <div className="d-flex">
                    ...
                    <div className="ms-3">
                      ...
                      <h6 className="h4 fw-normal mb-0">
                        {pendingTokens > 0
                          ? parseFloat(pendingTokens) / 10 ** 18
                          : 0}
                      </h6>
                    </div>
                  </div>
