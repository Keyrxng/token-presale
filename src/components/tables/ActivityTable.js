...
import { formatData, formatDate, toEther } from '../../lib/utils'
      ...
      <div row={row}>
        <Link to={`/users/${row.account}`} className="text-reset">
          <div className="d-flex -align-items-center">
            ...
            <p className="ms-2 mb-0 f2-bold">{row.name}</p>
          </div>
        </Link>
      </div>
    ),
  ...
  },
  {
    ...
    cell: (row) => (
      <div row={row}>
        {row.tokenPrice > 0 ? (
          <p className="mb-0">
            {(toEther(parseFloat(row.tokenPrice)) * row.amount) / 10 ** 18}
          </p>
        ) : (
          <p className="mb-0">-</p>
        )}
  ...
  useEffect(() => {
    if (tokenCtx.activity && userCtx.usersList) {
      const activities = tokenCtx.activity.map((item) => {
        return {
          account: item[0],
          name: userCtx.usersList.filter((user) => user.account === item[0])[0]
            .fullName,
          amount: item[1],
          tokenPrice: item[2],
          time: item[3],
